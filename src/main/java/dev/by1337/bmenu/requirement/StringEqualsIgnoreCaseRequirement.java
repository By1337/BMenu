...
public class StringEqualsIgnoreCaseRequirement implements Requirement {
    ...
    public StringEqualsIgnoreCaseRequirement(String input, String output, boolean not, Commands commands, Commands denyCommands) {
        this.input = input;
        this.output = output;
        this.not = not;
    ...
    public boolean test(Menu menu, PlaceholderApplier placeholder, Player clicker) {
        boolean b = placeholder.setPlaceholders(input).equalsIgnoreCase(placeholder.setPlaceholders(output));
        return not != b;
    }
